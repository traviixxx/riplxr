#!/usr/bin/env python3
"""
XRP HVN/LVN zone-touch trader (SIM or LIVE Binance Futures).
Volume profile logic aligned with v18_advanced_v2 (4h/1d wide profile).
Price poll: 3s | Profile refresh: 3min
Logs/state: XRPUSDT_RRCAO/XRP_hvn_lvn_* | Engine: XRPUSDT

LIVE: export XRP_LIVE_TRADE=true BINANCE_API_KEY=... BINANCE_ED25519_PRIVATE_KEY_PATH=...
      export XRP_ORDER_USDT=1000   # margin (Binance "Cost") USDT
      export XRP_LEVERAGE=40       # position notional ≈ 1000 × 40 = 40,000 USDT
      export XRP_ENTRY_ORDER_TYPE=LIMIT
      export XRP_LIMIT_ENTRY_OFFSET=0
      export XRP_ENTRY_ORDER_TIMEOUT_SEC=90
      # chỉ notional 1000 (không × lev): export XRP_SIZING_MODE=notional
"""
...
# CONFIG
# =========================
SYMBOL = os.getenv("XRP_SYMBOL", "XRPUSDT")
FUTURES_BASE = "https://fapi.binance.com"

ZONE_TOUCH_TOLERANCE = env_float("XRP_ZONE_TOUCH_TOLERANCE", "0.003", "ZONE_TOUCH_TOLERANCE")
TP_LVN_POINTS = env_float("XRP_TP_LVN_POINTS", "0.006", "TP_LVN_POINTS")
TP_HVN_POINTS = env_float("XRP_TP_HVN_POINTS", "0.012", "TP_HVN_POINTS")
SL_POINTS = env_float("XRP_SL_POINTS", "0.010", "SL_POINTS")
...
STRICT_EDGE_ONLY = env_bool("XRP_STRICT_EDGE_ONLY", True)
ENABLE_MAX_INVERSE = env_bool("XRP_ENABLE_MAX_INVERSE", True)
ENABLE_REJECTION_FOLLOW_TREND = env_bool("XRP_ENABLE_REJECTION_FOLLOW_TREND", True)
ENABLE_SHORT_BEARISH_4H_MIN_EDGE = env_bool("XRP_ENABLE_SHORT_BEARISH_4H_MIN_EDGE", True)
MAX_RETEST_IMPULSE_POINTS = float(os.getenv("XRP_MAX_RETEST_IMPULSE_POINTS", "0.02"))
RETEST_LOOKBACK_CANDLES = int(os.getenv("XRP_RETEST_LOOKBACK_CANDLES", "8"))
REJECTION_LOOKBACK_CANDLES = int(os.getenv("XRP_REJECTION_LOOKBACK_CANDLES", "3"))
REJECTION_WICK_RATIO = float(os.getenv("XRP_REJECTION_WICK_RATIO", "1.2"))
...
STATE_PATH = os.path.join(
    _SCRIPT_DIR,
    os.getenv("XRP_HVN_LVN_STATE", "XRP_hvn_lvn_trade_state.json"),
)
TRADE_LOG_PATH = os.path.join(
    _SCRIPT_DIR,
    os.getenv("XRP_HVN_LVN_LOG", "XRP_hvn_lvn_trades.jsonl"),
)

PRICE_DECIMALS = int(os.getenv("PRICE_DECIMALS", "4"))

LIVE_TRADE = env_bool("XRP_LIVE_TRADE", False)
...
ORDER_USDT = float(os.getenv("XRP_ORDER_USDT", os.getenv("XRP_MARGIN_USDT", "1000")))
LEVERAGE = int(os.getenv("XRP_LEVERAGE", "40"))
# margin (default): notional = ORDER_USDT * LEVERAGE | notional: ORDER_USDT only
SIZING_MODE = os.getenv("XRP_SIZING_MODE", "margin").strip().lower()
_ORDER_QTY_RAW = os.getenv("XRP_ORDER_QTY", "").strip()
...
ENTRY_ORDER_TYPE = os.getenv("XRP_ENTRY_ORDER_TYPE", "LIMIT").strip().upper()
LIMIT_ENTRY_OFFSET = float(os.getenv("XRP_LIMIT_ENTRY_OFFSET", "0"))
ENTRY_ORDER_TIMEOUT_SEC = float(os.getenv("XRP_ENTRY_ORDER_TIMEOUT_SEC", "90"))
...
BRACKET_MODE = os.getenv("XRP_BRACKET_MODE", "AUTO").strip().upper()
BRACKET_CLOSE_POSITION = env_bool("XRP_BRACKET_CLOSE_POSITION", True)
BRACKET_WORKING_TYPE = os.getenv("XRP_BRACKET_WORKING_TYPE", "MARK_PRICE").strip().upper()
POSITION_SIDE_MODE = os.getenv("BINANCE_POSITION_SIDE_MODE", "AUTO").strip().upper()  # AUTO | ONEWAY | HEDGE
BRACKET_RETRY_ATTEMPTS = max(1, int(os.getenv("XRP_BRACKET_RETRY", "1")))
BRACKET_RETRY_SLEEP_SEC = float(os.getenv("XRP_BRACKET_RETRY_SLEEP_SEC", os.getenv("XRP_BRACKET_RETRY_SEC", "3")))
BRACKET_RETRY_SEC = float(os.getenv("XRP_BRACKET_RETRY_SEC", str(BRACKET_RETRY_SLEEP_SEC)))
POSITION_STATUS_SEC = float(os.getenv("XRP_POSITION_STATUS_SEC", str(PRICE_POLL_SEC)))
# Binance USD-M now requires conditional TP/SL via Algo Order API on some accounts/API versions.
BRACKET_API = os.getenv("XRP_BRACKET_API", "ALGO").strip().upper()  # ALGO or ORDER
...
def bracket_levels(entry, side, zone_kind, sl_points=None, tp_points=None):
    """SL/TP từ entry thực (giá XRP, đơn vị USD)."""
    entry = float(entry)
    side = side.upper()
...
def open_trade(signal, price, cycle_ts):
    if LIVE_TRADE:
        if not live_trading_ready():
            print(
                f"[{cycle_ts}] ⚠️ XRP_LIVE_TRADE=true nhưng thiếu BINANCE_API_KEY hoặc Ed25519 PEM — bỏ qua lệnh"
            )
            return None
...
def run():
    ...
    if LIVE_TRADE and live_trading_ready():
        ...
        mode_label = "SIM"
    print("=" * 70)
    print(f"XRP HVN/LVN {mode_label} | {SYMBOL} | poll={PRICE_POLL_SEC}s | profile={PROFILE_REFRESH_SEC}s")
    if ENV_FILES_LOADED:
    ...
    if LIVE_TRADE:
        ex = f" ~{calc_order_qty(1.5)[0]} XRP @1.5" if ORDER_QTY_FIXED is None else f" fixed_qty={ORDER_QTY_FIXED}"
        margin_note = (
